...
def optimize_parameters(
    ...
    def objective(trial: optuna.Trial) -> float:
        fast_ema = trial.suggest_int("fast_ema", 3, 30)
        slow_ema = trial.suggest_int("slow_ema", fast_ema + 5, 120)
        signal_ema = trial.suggest_int("signal_ema", 3, 30)
        rsi_period = trial.suggest_int("rsi_period", 5, 40)
        rsi_entry_floor = trial.suggest_int("rsi_entry_floor", 20, 65)
        rsi_overheat_exit = trial.suggest_int("rsi_overheat_exit", rsi_entry_floor + 5, 95)
    ...
    return {key: int(value) for key, value in study.best_params.items()}
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.divider()
        st.header("Optuna 優化")
        n_trials = st.slider("試算次數", min_value=20, max_value=300, value=80, step=20)
    ...
    try:
        ...
        with st.spinner("Optuna 正在搜尋最佳參數..."):
    ...
    st.subheader("Optuna 最佳參數")
    param_cols = st.columns(6)
    param_cols[0].metric("快線 EMA", best_params["fast_ema"])
    param_cols[1].metric("慢線 EMA", best_params["slow_ema"])
    param_cols[2].metric("訊號線 EMA", best_params["signal_ema"])
    param_cols[3].metric("RSI 週期", best_params["rsi_period"])
    param_cols[4].metric("RSI 進場下限", best_params["rsi_entry_floor"])
    param_cols[5].metric("RSI 過熱出場", best_params["rsi_overheat_exit"])
